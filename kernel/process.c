    ...
    switch( parent->mapped_info[i].seg_type ){
      ...
      case HEAP_SEGMENT:
        ...
        // when mapping the heap blocks
        {
          ...
          // share heap blocks with COW: map child to parent's physical pages
          for (uint64 heap_block = parent->user_heap.heap_bottom;
            ...
            if (!parent_pa || !parent_pte || !(*parent_pte & PTE_V))
              panic("do_fork: invalid parent heap mapping at va:%lx\n", heap_block);

            // parent and child both become read-only COW mappings.
            uint64 cow_flags = (PTE_FLAGS(*parent_pte) & ~(PTE_W | PTE_D)) | PTE_COW;
            *parent_pte = PA2PTE(parent_pa) | cow_flags | PTE_V;
            user_vm_map((pagetable_t)child->pagetable, heap_block, PGSIZE, parent_pa, cow_flags);
            page_ref_inc((void*)parent_pa);
          }
          ...
          // parent page table entries changed from writable to read-only COW.
          flush_tlb();
          break;
        }
      case CODE_SEGMENT:
