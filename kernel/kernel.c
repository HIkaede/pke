...
void load_user_program(process *proc, uint64 hartid) {
  ...
  // USER_TRAP_FRAME_FOR_HART is a physical address defined in kernel/config.h
  proc->trapframe = (trapframe *)USER_TRAP_FRAME_FOR_HART(hartid);
  memset(proc->trapframe, 0, sizeof(trapframe));
  // USER_KSTACK_FOR_HART is also a physical address defined in kernel/config.h
  proc->kstack = USER_KSTACK_FOR_HART(hartid);
  proc->trapframe->regs.sp = USER_STACK_FOR_HART(hartid);
