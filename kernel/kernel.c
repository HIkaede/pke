...
static volatile int g_kernel_init_barrier = 0;
...
int s_start(void) {
  ...
  write_csr(satp, 0);
       ...
       // global kernel memory initialization should be done only once.
       if (hartid == 0) {
       ...
       // all harts wait for kernel memory initialization.
       sync_barrier(&g_kernel_init_barrier, NCPU);
