...
static volatile int g_platform_init_barrier = 0;
...
void m_start(uintptr_t hartid, uintptr_t dtb) {
  // make hartid available via tp in both S-mode and trap handlers.
  write_tp(hartid);

  // init shared platform state exactly once.
  if (hartid == 0) {
  ...
  sync_barrier(&g_platform_init_barrier, NCPU);
